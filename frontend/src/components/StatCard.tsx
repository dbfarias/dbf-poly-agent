...
export default function StatCard({ title, value, subtitle, icon, trend, testId, help }: StatCardProps) {
  return (
    <div className="bg-[#1e2130] rounded-lg border border-[#2a2d3e] p-4" data-testid={testId ? `stat-card-${testId}` : undefined}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm text-zinc-400 flex items-center" data-testid={testId ? `stat-title-${testId}` : undefined}>
          {title}
